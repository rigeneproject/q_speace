...
All gates are returned as dense 2^n x 2^n complex matrices. Single-qubit
gates are constructed via tensor product against identity on the
remaining qubits, with a `target` qubit index.
"""
...
class QuantumGates:
    ...
    Use single-qubit gates via `single_qubit(gate, num_qubits, target)`,
    and two-qubit gates via `two_qubit(gate, num_qubits, control, target)`.
    """
    ...
    def single_qubit(
        ...
        if not (0 <= target < num_qubits):
            raise ValueError(
                f"target {target} out of range [0, {num_qubits})"
            )
        base = QuantumGates._base_single(gate, angle)
        # tensor product: I^{target} ⊗ base ⊗ I^{num_qubits-1-target}
        mats = [_I2] * num_qubits
        mats[target] = base
        result = mats[0]
        for m in mats[1:]:
    ...
    def two_qubit(
        ...
        if not (0 <= control < num_qubits) or not (0 <= target < num_qubits):
            raise ValueError(
                f"control/target out of range: c={control} t={target}"
            )
        if control == target:
    ...
    @staticmethod
    def _base_single(gate: GateType, angle: Optional[float]) -> np.ndarray:
        if gate == GateType.H:
            return _H.copy()
        if gate == GateType.X:
            return _X.copy()
    ...
    def _cnot(num_qubits: int, control: int, target: int) -> np.ndarray:
        # Build CNOT in the full 2^n Hilbert space.
        size = 1 << num_qubits
    ...
    def _controlled_rotation(
        ...
        for idx in range(size):
            ...
            if c_bit == 0:
                ...
                t_idx_in_2 = t_bit
                new_t = 1 - t_bit
                # rows: source idx where t-bit flipped
                row = idx ^ (1 << (num_qubits - 1 - target))
                mat[row, idx] = base[new_t, t_idx_in_2]
        return mat
